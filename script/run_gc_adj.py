...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', '-m', type=str, default="PE", help='Single-end(SE) or Paired-end(PE) (default: PE)')
    ...
    os.system("python /dssg/home/zhangjp/soft/AECT/AECT-master/script/correctGCBias_for_bedtools.py -b %s --effectiveGenomeSize %s -g %s --GCbiasFrequenciesFile %s.freq.txt -o %s.gc.bam -p %s"
              % (args.bam, args.effectiveGenomeSize, args.genome2bit, args.bam, args.bam, args.threads))
    os.system("python /dssg/home/zhangjp/soft/AECT/AECT-master/script/computeBEDcov.py -b %s.gc.bam -d %s -m %s" % (args.bam, args.bed, args.mode))
